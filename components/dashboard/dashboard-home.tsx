...
export function DashboardHome() {
  ...
  } : { overallScore: 0, scores: { cyberThreat: { score: 0, label: "Cyber Threat Score", justification: "" }, vulnerability: { score: 0, label: "Vulnerability Score", justification: "" }, darkWeb: { score: 0, label: "Dark Web Score", justification: "" }, thirdParty: { score: 0, label: "Third-Party Risk Score", justification: "" }, reputation: { score: 0, label: "Reputation Score", justification: "" } } }
  ...
  useEffect(() => {
    const fetchCompanyData = async () => {
      try {
        ...
        try {
          const companiesResponse = await fetch(`${getBackendUrl()}/api/v1/user/${uid}/companies`)
          if (companiesResponse.ok) {
            ...
            if (companiesResult.status && companiesResult.body?.length > 0) {
              ...
              if (activeCompany?.company_id) {
                ...
                const lookupResponse = await fetch(`${getBackendUrl()}/api/v1/company/by-domain/${encodeURIComponent(activeCompany.company_domain)}`)
        ...
        try {
          const onboardingResponse = await fetch(`${getBackendUrl()}/api/v1/company/onboarding/${uid}`)
          ...
          const lookupResponse = await fetch(`${getBackendUrl()}/api/v1/company/by-domain/${encodeURIComponent(domain)}`)
        ...
        <Card className="lg:col-span-1 bg-card border-border shadow-md">
          <CardContent className="pt-6 text-center">
            ...
            <div className="mt-4 pt-4 border-t border-border text-sm text-muted-foreground">
              Weighted: Threat 8%, Vuln 20%, Dark Web 22%, Breach 15%, 3P 17%, Rep 18%
            </div>
          </CardContent>
        </Card>
